...
- agent-terraform                              | Claude Code skills + Python (FastMCP) | 2026-07-06 | spec-driven pipeline making internal APIs agent-consumable: analyze (code/wiki → `spec.yaml`) → 8-dim readiness gate → generate per-service MCP server + Agent Skill. Scaffold complete (5 skills, spec format, rubric, FastMCP template, fictional example spec); `.gravity` adopted v1.4 (root `CLAUDE.md` is the router; `spec` + `readiness` domains, each SPEC-fenced; MISSION in `.gravity/`); PR #1 open. Decided: per-service MCP. Next: merge PR #1, then Phase 1 — onboard the first real backend (small Python service) end-to-end; expect spec format + rubric revisions on first contact. Repo: github.com/mostlytricks/agent-terraform (clone into `repos/agent-terraform/` + junction on the workspace PC)
