...
- forge-world _(alias `forge`)_                | JS (Vite 8 + three r185)             | 2026-07-11 | game-feel **semiconductor stack-up simulator** (educational: slate = process layer, etch/deposit). PR #1 merged; **initial arc complete (Phases 1–4 ✓)**: process re-theme, pattern editor (custom types + self-contained stack@2 JSON), and soft ordering guidance (6-rule audit, PLAUSIBLE/ORDER-HINTS readout) all shipped on restarted branch `claude/3d-block-stacking-viz-rc0kxb` (remote `mostlytricks/forge-world`; junction/tier wiring pending on the local machine). Next: user picks the next arc — recipe presets · teaching mode · Electron wrapper · grid decision (`/interview forge <feature>`)
