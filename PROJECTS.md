...
- forge-world _(alias `forge`)_                | JS (Vite 8 + three r185)             | 2026-07-11 | game-feel **semiconductor stack-up simulator** (educational: slate = process layer, etch/deposit). PR #1 merged; Phases 2 (process re-theme) + 3 (pattern editor: paint on blueprint, custom types, self-contained stack@2 JSON) shipped on restarted branch `claude/3d-block-stacking-viz-rc0kxb` (remote `mostlytricks/forge-world`; junction/tier wiring pending on the local machine). Next: confirm Phase 4 review block (soft ordering guidance: ~6 rules, manifest glyphs + plausibility readout) then build it
