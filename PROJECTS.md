...
- forge-world _(alias `forge`)_                | JS (Vite 8 + three r185)             | 2026-07-11 | game-feel **semiconductor stack-up simulator** (educational: slate = process layer, etch/deposit). PR #1 merged (prototype + docs); Phase 2 process re-theme shipped on restarted branch `claude/3d-block-stacking-viz-rc0kxb` (10 process layers, wafer-build demo, µm readout; remote `mostlytricks/forge-world`; junction/tier wiring pending on the local machine). Next: confirm Phase 3 review block (pattern editor: fork built-ins, stack@2 JSON, cell painting) then build it
