...
- forge-world _(alias `forge`)_                | JS (Vite 8 + three r185)             | 2026-07-11 | game-feel **semiconductor stack-up simulator** (educational: slate = process layer, engrave = etch, relief = deposit). Phase 1 prototype verified; interviewed 2026-07-11 → four-doc pipeline minted (MISSION + PLAN). On branch `claude/3d-block-stacking-viz-rc0kxb` (remote `mostlytricks/forge-world`; junction/tier wiring pending on the local machine). Next: confirm Phase 2 review block (process-layer set, etch/deposit semantics) then build the re-theme
