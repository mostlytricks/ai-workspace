...
- agent-terraform                              | Claude Code skills + Python (FastMCP) | 2026-07-06 | spec-driven pipeline making internal APIs agent-consumable: analyze (code/wiki → `spec.yaml`) → 8-dim readiness gate → generate per-service MCP server + Agent Skill. Scaffold complete (5 skills, spec format, rubric, FastMCP template, fictional example spec); `.gravity` adopted v1.4 (root `CLAUDE.md` is the router; `spec` + `readiness` + `onboarding` domains, SPEC-fenced; MISSION in `.gravity/`); PRs #1-2 merged. Decided: per-service MCP. Next: Phase 1 pilot **on the work machine** (Claude Code, Opus 4.8+) — fill `.gravity/onboarding/PLAN.md`'s preparation checklist (small Python service ≤ ~15 endpoints, doc exports, auth facts), then `onboard <name>, code at <path>, docs at <path>`; expect spec format + rubric revisions on first contact. Repo: github.com/mostlytricks/agent-terraform (clone into `repos/agent-terraform/` + junction on the workspace PC)
