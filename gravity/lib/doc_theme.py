...
`install_lib.py` copies into `<project>/.gravity/lib/`, so a clone that has
...
def css() -> str:
    ...
    status vocabulary, switcher. No `@font-face`, no CDN — a doc opened from
    `file://` on a plane still renders."""
