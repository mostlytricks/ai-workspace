...
> **alias:** `<short-name>` — optional per-project short name; gravity tooling (`.kepler/scripts/resolve_project.py`) resolves it so you can type it instead of the full folder name. Delete this line to fall back to the derived acronym (e.g. `architecture-memory-os` → `amos`).
...
<!-- A fresh agent opening here gets only the auto-loaded CLAUDE.md + CONTEXT.md; this map
     ...
     don't apply — two-doc projects keep only the first two.
     Precedence: CONTEXT (now) > CLAUDE (how) > PLAN (next) > MISSION (why).
     `.gravity/` projects: DELETE everything below the first two lines — the fenced router
     block above already points to .gravity/GRAVITY.md (the protocol card) and
     .gravity/ROUTER.md (the Doc Map); don't maintain a second map here. -->
...
- **ARCHITECTURE.html** — how it's built (only if present; else *Entry Points* below owns it). *(optional fifth doc)*
- **DESIGN.md** — running-app visual design system (only if present; UI projects). / **RUNBOOK.md** — deploy, health checks, rollback (only if present; deploying projects). Each stencil self-describes in its header.
...
<!-- This is the lightweight architecture map and the default home for "how it's built".
     If the project grows an ARCHITECTURE.html, that file becomes the canonical "how it's built" —
     keep this section a pointer to it rather than duplicating boundaries. -->
