<!--
INBOX.template.md — the drop-zone door. Copy to <project>/.gravity/_inbox/README.md
...
.gitignore keeps it visible in clones (`.gravity/_inbox/*` + `!.gravity/_inbox/README.md`) while
...
|---|---|
| **DB evidence** — DDL scripts you scraped (`CREATE TABLE` from SQL Developer / DBeaver / migrations) or the metadata CSVs a DBA exports | `.gravity/integration/structural/db/` (DDL under `ddl/`) beside its `MANIFEST.md`; then `python .gravity/_lib/scan_db.py` reads it. No row data is ever needed |
...
- **The inbox ends empty** — after routing, a file lives in a `_given/` layer
