...
- **Domain (`.gravity/`)** — a durable subject area an agent repeatedly navigates and changes, with its own *principle* and non-goal — earns a `.gravity/<domain>/` folder. Not every feature is a domain; a one-off is a `PLAN.*.md` slice under an existing domain. The *is-it-a-domain?* gate lives in the project's `.gravity/ROUTER.md` (pre-v3: root `CLAUDE.md`). Positioning against the DDD term: a gravity domain is a **documentation facet**, smaller and looser than a DDD *bounded context* — sometimes they coincide, but brownfield domains are often just discovered modules.
