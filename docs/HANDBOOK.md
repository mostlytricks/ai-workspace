...
| `/interview <name>` | `/mission` in reverse: interview *you* to fill a project's docs. Gap-scans what exists, asks the five themes (telos, walls, shape, gate, now) strawman-first, writes each answer to its owner-doc, leaves unresolved items as visible `OPEN:` lines. For new projects ("fill in CLAUDE.md") and stuck ones (open questions blocking a phase). Doesn't commit. |
