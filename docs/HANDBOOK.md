...
| `/given <name>` | Route `.gravity/inbox/` into the given layer: one routing table, provenance manifests, inbox ends empty. |
...
| `/observatory <name> [theme]` | One project, one page: seven tabs (Overview+drift+tracks · Queue · Seams · Spec Health · Graduation · Timeline · Orbit 3D) rendered into `<project>/.gravity/observatory/` — generated, git-ignored, and self-renderable off-workspace (`python .gravity/lib/generate_observatory.py`). |
