...
| `/adopt-gravity <name>` | Retrofit the `.gravity/` doc system into a doc-heavy project: relocate the heavy docs out of the root into per-domain `.gravity/` folders (`git mv`), seed `.gravity/ROUTER.md` + fence the thin router block into the root harness files, wire the four registry owners. Proposes the move table and confirms before touching disk; doesn't commit. |
...
| `/observatory <name> [theme]` | **One project, one page** — THE per-project view, seven tabs over a single scan (`gravity/lib/scan_project.py` — one scanner, many instruments): **Overview** (goal · CONTEXT now/next · the drift card — live `check.py` findings · spine table · the **tracks card**, the direction axis · doc links) · **Queue** (every `PLAN.*.md` as one work table — building first, shipped dimmed, track ⟡ chips) · **Seams** (the integration Boundary Map as a live graph: rows become flowing edges, packets = direction, evidence citations per seam, `OPEN:` rows dashed guard-red, unparseable rows listed loud never guessed; honest empty state when no integration SPEC) · **Spec Health** (per-domain walls vs `[review]` judgment, gate presence, test-bound Behavioral Contract lines, `FILL` leftovers; freeform SPECs get a labeled tag census) · **Graduation** (PLAN scenarios vs SPEC Behavioral Contract — ✔ graduated / ○ still intent / the two dishonesty smells loud) · **Timeline** (`docs/walkthroughs/` as a reverse-chron proof strip) · **Orbit 3D** (drag-to-orbit analytical view: **coupling arcs** from path-shaped doc cross-references, **track arcs** — ⟡ dashed labeled chains over one cross-domain direction, **health rings** — solid arc = walls share, **guard-red pulse** on unfenced ◑ domains, comet trails <7d, HUD toggles). Nav carries **attention badges**; tabs are hash-deep-linkable. **Live theme buttons** switch chrome + instruments in place (persisted under the workspace-wide `dash-theme` key; `--theme` is only the first-load default; `aurora`/`daylight`/`sandstone`/`forest`/`slate`). **Output lands inside the project** at `<project>/.gravity/observatory/index.html` — a folder that ignores itself (`.gitignore` of `*`), so the page is visible to anyone who opens the repo but never becomes a tracked artifact that can go stale; doc links are relative, so it isn't pinned to the machine that rendered it. The renderer is protocol-side (`gravity/lib/`), so a project carrying `.gravity/lib/` renders itself with **no workspace and no arguments**: `python .gravity/lib/generate_observatory.py`. The old `/cosmos` + `/boundary` commands were folded in here (generators live on as renderer modules with debug CLIs writing beside the page); the 2D Domains tab retired 2026-07-25 — the 3D view superseded it. |
...
The four-doc pipeline above keeps the docs at the project root. Once a project grows **several** architecture/spec/plan docs across **multiple domains**, the root gets crowded and the two files that auto-load (`CLAUDE.md`, `CONTEXT.md`) get buried. The `.gravity/` system (workspace `CLAUDE.md` §6) fixes that: it relocates every heavy doc into a `.gravity/` directory **organized by subject domain**, leaving only `CLAUDE.md` + `CONTEXT.md` + `README.md` at the root. The **router** — a Doc Map plus a "what to read before which change" table — lives at `.gravity/ROUTER.md`; the root harness files (`CLAUDE.md`, `AGENTS.md`, …) carry only gravity's 4-line fenced pointer block, so adoption never collides with a project's own root content (pre-v3 projects carry the router in root `CLAUDE.md`). `knowledge-viewer` is the worked example.
...
**Retrofit an existing project** — run `/adopt-gravity <name>`. The agent inventories the root, proposes a before→after move table (grouping docs by domain — *you confirm the boundaries*), `git mv`s them into `.gravity/<domain>/` folders, fixes cross-references, seeds `.gravity/ROUTER.md` from `ROUTER.template.md` + inserts the fenced block (`GRAVITY.template.md`) into the root harness files, embeds the **protocol card** (`.gravity/GRAVITY.md`, copied from `gravity/GRAVITY-PROTOCOL.md` and stamped from `VERSION` — the project-side protocol that travels with the repo, so an agent opening the project *without* the workspace still knows how to work `.gravity/`), installs the **lib** (`.gravity/lib/`, copied from `gravity/lib/` and stamped — stdlib-only instruments so the repo can render and check *itself* off-workspace), and wires the four registry owners (directory · CLAUDE.md routing · MISSION why-rows · IMPLEMENTATION_PLAN status). It stops to confirm before moving anything and doesn't commit.

**Add a domain later** — run `/new-domain <name> <domain>`. It runs the *is-it-a-domain?* gate first (most features are a `PLAN.*.md` slice under an existing domain, not a new folder), then creates `.gravity/<domain>/PLAN.md` and **wires all four indexes** so the domain is never orphaned. The full gate + lifecycle lives in the project's `.gravity/ROUTER.md` "Adding a domain" section (pre-v3: root `CLAUDE.md`).
...
Gravity itself is versioned (`VERSION` + `CHANGELOG.md` + git tag), and each adopted project pins the version it was built on in **three stamps**: the `> gravity: vX.Y` stamp in its fenced router block (root `CLAUDE.md`/`AGENTS.md`), and — for `.gravity/` projects — the `gravity protocol · vX.Y` stamp in the embedded card (`.gravity/GRAVITY.md`) plus the plain version in `.gravity/lib/VERSION` (`LIB_STALE` when it trails the distribution). When gravity cuts a new release, projects don't upgrade themselves; they drift, visibly: `/triage` flags stale cards (📡), and the `PROJECTS.md` **Gravity adoption** table shows who's on what.

**Run `/sync-gravity <name>`** to bring one project current. It does the two layers differently on purpose:

- **Mechanical (applied for you):** re-copies the protocol card fresh from the template (it's a verbatim copy by contract — never hand-merged), re-installs `.gravity/lib/` (`install_lib.py` — same verbatim contract, so upgrading means re-copying, never patching), bumps both stamps to the current `VERSION`, verifies with `check.py consistency`, and reconciles the adoption-table row.
...
- **Work layers (slice · phase · spine · track)** — the law that keeps plan vocabulary from blurring: the **slice** is the *only unit of work* — the smallest shippable change that passes the gate, its intent living in a domain `PLAN.*.md`. Everything else in `IMPLEMENTATION_PLAN.md` is an **index over slices** on one of three axes: **time** (the phase roadmap or now/next/later queue — a *phase* is just a slice with an ordinal, a lane just a slice with a priority), **domain** (the `✓/◑/○` status spine), and **direction** (**tracks** — named cross-domain intents, ≤3 active, each pointing up to a MISSION principle and down to the carrying slices, retired once their residue is enforcement-tagged SPEC rules). Indexes hold no work: work items written into the spine or a track row belong in a slice PLAN, linked from there. Full statement in `IMPLEMENTATION_PLAN.template.md`; the observatory renders all three axes (Queue = time, spine = domain, tracks card + Orbit 3D track arcs = direction).
- **Chore** — small maintenance work with no principle of its own; **never a domain** (the same ruling as bugs). Handled immediately → no doc at all — writing it down would be ceremony. Deferred → **one dated `○` row** in the `IMPLEMENTATION_PLAN.md` queue (`<chore> (deferred YYYY-MM-DD)`), earning a slice PLAN only at pickup — an unwritten deferral doesn't exist. The date is the **comet clock**: `check.py consistency` WARNs `SLICE_STALE` on deferred rows and `○` slice PLANs older than 30 days, so a delayed chore resurfaces by age, never by memory (the fix is pick it up, re-date it, or drop it — silence is the one wrong move). A **recurring** chore is a missing wall: graduate it into a tagged SPEC rule, a gate step, or a `RUNBOOK.md` line, never into a folder.
...
- **Protocol card (`.gravity/GRAVITY.md`)** — the project-embedded, project-side subset of the gravity protocol (doc kinds + rates, navigation discipline, SPEC anatomy). Copied verbatim from `gravity/GRAVITY-PROTOCOL.md` whenever `.gravity/` is created, stamped `gravity protocol · vX.Y` from the workspace `VERSION`, never hand-edited (upgrade = re-copy). It makes each project repo **self-describing**: an agent that clones the project without the workspace still learns how to work `.gravity/`. Workspace rules (tiers, junctions) are never embedded. `/triage` flags a missing or stale card (📡).
- **Domain (`.gravity/`)** — a durable subject area an agent repeatedly navigates and changes, with its own *principle* and non-goal — earns a `.gravity/<domain>/` folder. Not every feature is a domain; a one-off is a `PLAN.*.md` slice under an existing domain. The *is-it-a-domain?* gate lives in the project's `.gravity/ROUTER.md` (pre-v3: root `CLAUDE.md`). **Two axes, capability first:** vertical (business/capability) domains — the units of purpose a user scenario names — are the default diagnosis; horizontal/structural ones (`data`, `security`, `ops`) earn folders only where a runtime owns real rules worth fencing. One-folder-per-service is the degenerate case: "it's a separate repo/deployable" is not a principle, and that topology already lives in `integration`'s Boundary Map. Positioning against the DDD term: a gravity domain is a **documentation facet**, smaller and looser than a DDD *bounded context* — sometimes they coincide, but brownfield domains are often just discovered modules.
- **`SPEC.md` — the change contract** — the per-domain agent-loadable contract: a Minimal Shape to build *from* plus enforcement-tagged Rules that fence it (copy from `SPEC.template.md`; anatomy in workspace `CLAUDE.md` §6). Positioning against industry "spec-driven development" (Spec Kit / Kiro-style): gravity is **spec-governed change, not spec-generated scaffolding** — the SPEC is a contract with named enforcement that bounds what agents do to a system, not a blueprint code is produced from.
...
- `GRAVITY.template.md` — the thin fenced router block for the root harness files; `ROUTER.template.md` — the full in-`.gravity/` router (Doc Map + read-first table + domain gate).
- `gravity/GRAVITY-PROTOCOL.md` — the protocol card copied to `.gravity/GRAVITY.md` at adoption; makes the project repo self-describing off-workspace.
