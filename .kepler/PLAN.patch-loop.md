...
- **[NEW]** `.kepler/PLAN.patch-loop.md` — this file: the ritual spec + Execution block
...
Minting `/patch-slice` + a `.kepler/scenarios/` fixture for it = the **next** slice,
...
- ~~OPEN: where the ritual lives long-term~~ **Resolved (2026-07-12, at minting):**
  the ritual lives in the `/patch-slice` command + the HANDBOOK cheat-sheet row.
  **No** Execution-block stencil in `PLAN.template.md` — the script writes and owns
  that block itself (`anchor`/`verify`/`rollback` append it), so a stencil would only
  invite hand-edited drift from the machine-written format.
...
**2026-07-06 — SCRIPT CORE BUILT: `.kepler/scripts/patch_slice.py`** (the weaker-agent
...
**2026-07-12 — MINTED: `/patch-slice` + the golden scenario.** The deferred wrapper
shipped (gravity v2.3 material):

- **`.claude/commands/patch-slice.md`** — the thin wrapper: sequences the six
  subcommands, owns only the two judgment steps (the patch, the re-plan prose), and
  carries the never-do list distilled from F3/F4 + the N=3/retention decisions.
- **Bug intake** (new rule, minted with the wrapper): a bug enters the slice PLAN's
  Scenario as a **currently-false** given/when/then — the repro — and the fix must
  leave the named regression test that graduates it into the SPEC's Behavioral
  Contract. Wired into `PLAN.template.md`, workspace CLAUDE.md §6, and the command.
- **`.kepler/scenarios/patch-slice/`** — fixture (`fixture-shop`: a green gate over a
  live untested clamp bug + a SPEC-declared stateful ledger; `PLAN.fix.md` is the
  bug-intake worked example) + SCENARIO.md replay recipe.
- **`check.py selftest` patch-loop half** — drives the script end-to-end over the
  fixture, both fork branches: green (preflight→anchor→snap→fix→verify→cleanup) and
  red (3 red verifies → exit 75 → four-proof rollback, gitignored ledger restored
  byte-identical, execution log surviving the hard reset). The "check.py covers the
  script" gap is closed.
- Also fixed: the script's `--help` crashed on cp949 consoles (stdout reconfigure
  guard added — the same F4-adjacent lesson: output plumbing must not lie or die).

## Next

None — the loop is closed (design → pilots → script core → skill + scenario). Future
findings from real `/patch-slice` runs append here as F9+.
