...
def _scan_workspace(root: Path | None):
    """Import the fact scanner (.kepler/scripts/scan_workspace.py) and run it.
...
def _resolve_project_arg(arg: str) -> str:
    ...
    dir, hand it to the shared resolver in .kepler/scripts/resolve_project.py."""
    if Path(arg).is_dir():
        return arg
    scripts = Path(__file__).resolve().parent.parent / "scripts"
    sys.path.insert(0, str(scripts))
...
def _theme_sources() -> dict[str, tuple[Path, str]]:
    ...
    return {
        ...
        "dashboard": (ws / ".kepler" / "dashboard" / "generate_dashboard.py", "css"),
...
def _patchloop_selftest() -> bool:
    """Drive .kepler/scripts/patch_slice.py end-to-end over the patch-slice
