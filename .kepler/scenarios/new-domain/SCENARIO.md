...
cp -r .kepler/scenarios/new-domain/fixture "$ACTUAL"
...
python .kepler/scenarios/check.py scenario \
    --scenario .kepler/scenarios/new-domain \
    --actual "$ACTUAL"
```
...
`python .kepler/scenarios/check.py selftest` proves `check.py` itself: the good fixture passes, and a copy with `model` stripped from the Doc Map is correctly caught as `UNDERWIRED`. Run it after editing `check.py`.
