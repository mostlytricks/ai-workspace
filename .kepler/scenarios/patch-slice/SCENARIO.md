...
**Command under test:** `/patch-slice <project> [slug]` (the thin wrapper over `.kepler/scripts/patch_slice.py`)
...
Replay this whenever you change `patch-slice.md` (the command), `patch_slice.py` (the walls), or the ritual itself (`.kepler/PLAN.patch-loop.md`).
...
cp -r .kepler/scenarios/patch-slice/fixture "$ACTUAL"
cd "$ACTUAL"
...
`python .kepler/scenarios/check.py selftest` drives `patch_slice.py` over this fixture end-to-end, both branches — including the exit-75 exhaustion wall and the four-proof rollback with a deliberately corrupted `state/data.txt` restored byte-identical. Run it after editing `patch_slice.py` or this fixture.
