...
ACTUAL="$(mktemp -d)/ws"
cp -r .kepler/scenarios/ship/fixture "$ACTUAL"
...
python - <<'EOF'
import sys, os
sys.path.insert(0, ".kepler/scenarios")
...
`python .kepler/scenarios/check.py selftest` proves the workspace checker itself: a healthy mini-workspace passes, and three seeded drifts (`MULTI_TIER`, `INDEX_MISSING_ON_DISK`, `MISSING_TRIGGER`) are each caught. Run it after editing `check.py` or `scan_workspace.py`.
