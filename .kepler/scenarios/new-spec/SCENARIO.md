...
cp -r .kepler/scenarios/new-spec/fixture "$ACTUAL"
...
python .kepler/scenarios/check.py scenario \
    --scenario .kepler/scenarios/new-spec \
    --actual "$ACTUAL"
```
...
`python .kepler/scenarios/check.py selftest` proves both checkers — including that every scenario's golden fixture is itself clean, and that a dead gate, dead tag, and template leftover are each caught. Run it after editing `check.py`.
