...
Then open .kepler/dashboard/dashboard.html in a browser. Rerun to refresh.

If the charts don't render, the vendored lib is missing — fetch it once:
    curl -fsSL -o .kepler/dashboard/vendor/chart.umd.min.js \
...
WORKSPACE_ROOT = SCRIPT_DIR.parent.parent          # .kepler/dashboard -> root
...
# A full trailing year, GitHub-esque — the workspace is old enough now that a
# 6-month window wasted the band's width (it hugged the left third of a 1920px
# panel) and cut the story short. Quartile heat thresholds come from all
# history, so widening the window never recolors the past.
CONTRIB_WEEKS = 53
...
def observatory_state(base: Path) -> dict:
    ...
    observatory never links back — a path to `.kepler/dashboard/` exists only
...
# --- purpose line + doc rail (live from repos/<name>/, not from PROJECTS.md) ---
# The roster's identity layer. PROJECTS.md's one-liner is *status* (focus /
# trigger / blocker — volatile by design); the stable "what is this project?"
# already lives as the first prose paragraph under the H1 of each project's
# CLAUDE.md (gravity's CLAUDE.template.md convention). Read live from disk,
# same doctrine as the adoption chips — it can't drift, and there is no second
# purpose field to keep in sync.
PURPOSE_MAX = 260

def project_purpose(name: str) -> str:
    """First prose line under the H1 of repos/<name>/CLAUDE.md, or ""."""
    base = WORKSPACE_ROOT / "repos" / repo_name(name)
    try:
        text = (base / "CLAUDE.md").read_text(encoding="utf-8", errors="ignore")
    except OSError:
        return ""
    in_fence = seen_h1 = False
    for raw in text.splitlines():
        ...
        if not seen_h1:
            continue
        # skip structure: blockquotes (alias/router stamps), comments, rules,
        # tables, list items, badges — the identity line is plain prose.
        if line.startswith((">", "<!--", "---", "|", "- ", "* ", "+ ", "![", "[![")):
            continue
        line = re.sub(r"\*\*|__", "", re.sub(r"\s+", " ", line))
        # An unfilled stencil is not a purpose — fall back to the status line
        # rather than parroting the template (same honesty as /triage's
        # stencil flags).
        if "<FILL" in line or line.startswith("One-sentence description"):
            return ""
        return line[:PURPOSE_MAX] + ("…" if len(line) > PURPOSE_MAX else "")
    return ""


# The doc rail: existence-checked links into the project's own browser-ready
# docs — the detail pages that already exist (authored MISSION/ARCHITECTURE,
# the protocol guide). Same one-way rule as the observatory chip: the fleet
# door links *into* a project, never the reverse.
DOC_RAIL = [
    ("mission", ("MISSION.html", ".gravity/MISSION.html")),
    ("architecture", ("ARCHITECTURE.html", ".gravity/ARCHITECTURE.html")),
    ("guide", (".gravity/GRAVITY.html",)),
]

def doc_links(name: str) -> str:
    rn = repo_name(name)
    base = WORKSPACE_ROOT / "repos" / rn
    chips = []
    for label, paths in DOC_RAIL:
        for p in paths:
            if (base / p).is_file():
                chips.append(f'<a class="gv doc" href="../../repos/{rn}/{p}" '
                             f'title="open {esc(rn)}&#39;s {label} page">{label}</a>')
                break
    return f'<div class="gvbar docs">{"".join(chips)}</div>' if chips else ""
    ...
    for tname, solid, _ in TIERS:
        for p in tiers[tname]:
            ...
            orbit.append({
                ...
                "purpose": project_purpose(p["name"])[:200],
                "slug": slugify(p["name"]),
            })
...
def render_roster(tiers: dict[str, list[dict]], today: date) -> str:
    """The calm zone: one ledger row per project, grouped by tier.

    Two zones, one page (DESIGN.dashboard.md §0): the instruments above stay
    loud and glassy; the roster below reads like a doc page — thin ruled rows.
    The collapsed row line is the *purpose* (identity, live from the project's
    CLAUDE.md — the stable fact you forget after months); the volatile status
    one-liner from PROJECTS.md moves into the expanded view, alongside the doc
    rail + adoption chips. `id="card-<slug>"` is the orbit-map planet's landing
    anchor — same slugify() feeds the payload, so planet↔row links can't drift.
    """
    sections = []
    for name, solid, grad in TIERS:
        ...
        for p in tiers[name]:
            ...
            purpose = project_purpose(p["name"])
            # No purpose (external path / no CLAUDE.md) → the status line takes
            # the row slot and isn't repeated in the detail.
            rowline = esc(purpose) if purpose else esc(p["focus"])
            status = (f'<div class="status"><span class="slab">status</span>'
                      f'{esc(p["focus"])}</div>') if purpose else ""
            detail_bits = status + doc_links(p["name"]) + adoption_chips(p["name"], tier=name)
            detail = f'<div class="detail">{detail_bits}</div>' if detail_bits else ""
            rows.append(
                f'<div class="row {cls}" id="card-{slugify(p["name"])}">'
                f'<div class="rowhead">{marker}<span class="pname">{esc(p["name"])}</span>'
                f'<span class="stack">{stack}</span>'
                f'<span class="ago">{ago}</span><span class="chev">⌄</span></div>'
                f'<div class="purpose">{rowline}</div>'
                f'{detail}</div>'
            )
        body = "".join(rows) if rows else '<div class="empty">— empty</div>'
        sections.append(
            f'<section class="tier" style="--accent:{solid};--grad:{grad}">'
            f'<h2>{name}/ <span class="tcount">{len(tiers[name])}</span></h2>'
            f'<div class="roster">{body}</div></section>'
        )
    # Explicit two-column split in attention order: active alone on the left
    # (the working column), the settled tiers stacked on the right. A CSS
    # column-balancer would split by height and wander; this break is meaning.
    return (f'<div class="fcol">{sections[0]}</div>'
            f'<div class="fcol">{"".join(sections[1:])}</div>')
...
def render(tiers: dict[str, list[dict]], today: date) -> str:
    ...
    out = out.replace("__ROSTER__", render_roster(tiers, today))
...
# Placeholders: __DATA_JSON__ __ROSTER__ __GENERATED__ __TOTAL__
...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  /* --- fleet roster: the calm zone. The instruments above keep the loud glass;
     down here the page shifts to a doc register — full-width ruled rows, no
     tiles, no lift. The focus line (a paragraph in PROJECTS.md) is clamped to
     one line; clicking a row expands it and reveals the adoption chips. --- */
  .fleet { animation:fadeIn .7s ease both; max-width:1060px; }
  /* wide screens: two explicit columns in attention order — active | the
     settled tiers (stable, dormant, archive). The split is authored in the
     generator (.fcol wrappers), never height-balanced, so the break can't
     wander; below the breakpoint the columns simply stack in the same order. */
  @media (min-width:1400px) {
    .fleet { max-width:none; display:grid; grid-template-columns:1fr 1fr;
  ...
  .tier h2 {
    ...
    letter-spacing:1.4px; color:var(--ink); margin:0 0 10px;
  ...
  .row {
    position:relative; padding:11px 10px 12px 17px; border-bottom:1px solid var(--border);
    cursor:pointer; --bar:var(--grad);
    transition:background .2s ease;
  }
  .row::before {
    content:''; position:absolute; left:0; top:12px; bottom:13px; width:3px;
    border-radius:2px; background:var(--bar);
  }
  .row:hover { background:var(--chip-bg); }
  .row.stale  { --bar:#FBBF24; }
  .row.vstale { --bar:#FF4D6D; }
  .rowhead { display:flex; align-items:baseline; gap:10px; }
  .pname { font-weight:600; font-size:13.5px; word-break:break-word; }
  .ago { margin-left:auto; color:var(--muted); font-size:11.5px; font-family:var(--mono); font-variant-numeric:tabular-nums; white-space:nowrap; }
  .marker { font-size:13px; }
  .chev { color:var(--muted); font-size:11px; transition:transform .2s ease; user-select:none; }
  .row.open .chev { transform:rotate(180deg); }
  .stack { font-size:10.5px; color:var(--muted); font-family:var(--mono); background:var(--chip-bg);
  ...
  .row.open .stack { max-width:none; white-space:normal; }
  .purpose { color:var(--focus-text); font-size:12.5px; line-height:1.55; margin-top:4px;
    display:-webkit-box; -webkit-line-clamp:1; -webkit-box-orient:vertical; overflow:hidden; }
  .row.open .purpose { display:block; -webkit-line-clamp:unset; overflow:visible; }
  .detail { display:none; }
  .row.open .detail { display:block; }
  .status { color:var(--muted); font-size:12px; line-height:1.55; margin-top:6px; }
  .status .slab { font-family:var(--mono); font-size:9px; text-transform:uppercase;
    letter-spacing:1.2px; color:var(--muted); opacity:.8; margin-right:8px; vertical-align:1px; }
  /* doc rail — navigation chips (theme chrome, not semantic status colors) */
  .gvbar.docs { margin-top:8px; }
  a.gv { text-decoration:none; }
  .gv.doc { color:var(--ring-a); border-color:var(--ring-a); cursor:pointer;
    opacity:.85; transition:opacity .15s, filter .15s; }
  .gv.doc:hover { opacity:1; filter:brightness(1.2); }
  .empty { color:var(--muted); font-style:italic; padding:8px 2px; border-bottom:1px solid var(--border); }

  /* landing highlight after a planet click scrolls to a row (auto-expanded) */
  .row.flash { background:var(--selection); }
  ...
  /* centered: on a wide panel the year-grid sits mid-band instead of hugging
     the left edge; on a narrow one, width:max-content keeps the x-scroll. */
  .contrib-wrap { width:max-content; min-width:max-content; margin:0 auto; }
...
<body>
  ...
  <div id="fallback">⚠ Charts library not found. Fetch it once:
    <code>curl -fsSL -o .kepler/dashboard/vendor/chart.umd.min.js https://cdn.jsdelivr.net/npm/chart.js@4.4.3/dist/chart.umd.min.js</code></div>
  ...
  <div class="fleet">__ROSTER__</div>

  <div class="cap" style="margin-top:20px">row line = the project's purpose (live from its CLAUDE.md) · click a row → status line, doc links (<span class="gv doc">mission</span> <span class="gv doc">architecture</span> <span class="gv doc">guide</span>) + gravity adoption (live from disk) ·
  ...
  <footer>Generated from PROJECTS.md by .kepler/dashboard/generate_dashboard.py · adoption chips read live from repos/&lt;name&gt;/ · design: DESIGN.dashboard.md · Chart.js + fonts vendored locally · rerun to refresh.</footer>
...
(function () {
  ...
  function showTip(p, target) {
    ...
    var body = p.purpose
      ? '<div class="t-focus">' + escHtml(p.purpose) + '</div>' +
        '<div class="t-meta" style="margin-top:5px">' + escHtml(p.focus) + '</div>'
    ...
    ORBITS.forEach(function (o, ti) {
      ...
      planets.forEach(function (p, i) {
        ...
        c.addEventListener('click', function () {
          var row = document.getElementById('card-' + p.slug);
          if (!row) return;
          row.classList.add('open');            // land with the full story showing
  ...
  // --- roster rows: click toggles the full focus line + adoption chips.
  // The observatory chip is a real link — let it navigate instead of toggling.
