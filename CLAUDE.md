...
| One domain's spec — what to build + the walls (the change contract) | **`.gravity/<domain>/SPEC.md`** — the compact agent-loadable sheet: a Minimal Shape to build *from* + enforcement-tagged Rules that *fence* it, links up to its facet | "is this the short spec you'd hand an *agent* to make a change safely?" |
...
**Optional: the `.gravity/` directory — where the heavy docs live (faceted projects).** Faceting (a deep-dive + an agent rule-sheet per domain) works, but the files pile up at the project root and bury the two that matter most. The fix is a **`.gravity/` directory** that holds *everything except* the two auto-loaders — `CLAUDE.md` and `CONTEXT.md` — plus `README.md` (the user guide). **Gravity owns nothing in those root files except a 4-line fenced `<!-- gravity:router -->` block** (identical in `CLAUDE.md`, `AGENTS.md`, and any other harness file; machine-managed by `/adopt-gravity`/`/sync-gravity`) — the full map lives one hop away in **`.gravity/ROUTER.md`**, so adoption never collides with a project's own root content.
...
```
<project>/
  CLAUDE.md · CONTEXT.md · README.md          # stay at root — identity (+ the fenced pointer), now, and the user guide
  .gravity/
    GRAVITY.md                                  # the embedded protocol card (versioned copy — see below)
    ROUTER.md                                   # the router — Doc Map + read-first table + the domain gate
...
- **Navigate** from the `.gravity/ROUTER.md` Doc Map (pre-v3 projects: root `CLAUDE.md`) — never guess paths.
...
- **Adding a feature?** Run the gate first — *is it a domain?* (its own principle + non-goal, rules worth a SPEC, a multi-step arc) or just a `PLAN.*.md` slice under an existing domain. Diagnose **capabilities-first**: vertical (business/capability) domains are the default axis; structural ones only where a runtime owns real rules — never one folder per service ("a separate deployable" is no principle; that topology is `integration`'s Boundary Map). Mint a folder only when it earns its own gravity, and **wire all four indexes** (Doc Map, router table, MISSION row, PLAN spine) so it's never orphaned. Chores are never a domain: done now → no doc; deferred → one dated `○` queue row (a slice PLAN only at pickup; `SLICE_STALE` flags 30-day comets); recurring → graduate to a wall (SPEC rule, gate step, RUNBOOK line), never a folder.
