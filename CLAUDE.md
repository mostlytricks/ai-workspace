...
│   ├── GRAVITY-PROTOCOL.md         #   the protocol card → copied verbatim to <project>/.gravity/GRAVITY.md at adoption (§6). For the AGENT.
│   ├── GRAVITY-GUIDE.html          #   its human twin → copied verbatim to <project>/.gravity/GRAVITY.html. Browser-read: what gravity does, the arc, the phrasebook. For the ENGINEER. (+ .ko sibling, same contract)
...
| Business-agreed future scope · stakeholder estimates | **`.gravity/_roadmap/ROADMAP.md`** — the plan sheet (chunks, not slices; URDs themselves are `_given/` evidence; `/urd` authors both) | "is this agreed with users but not yet code work?" |
...
<project>/
  ...
  .gravity/
    GRAVITY.md · GRAVITY.html                 # the protocol card (agent, canonical) + the engineer's guide (human, browser-read) — one pair
    ...
    _inbox/ · _lib/ · _observatory/ · _given/ · _roadmap/  # machinery — a leading `_` marks gravity's own dirs, never a domain
    <domain>/   ARCHITECTURE.html · SPEC.md · PLAN.*.md        # one folder per subject
```

**The card is the canon.** `.gravity/GRAVITY.md` is a verbatim, version-stamped copy of `gravity/GRAVITY-PROTOCOL.md`, seeded at adoption and refreshed by re-copy (`/sync-gravity`) — it makes each independent repo self-describing to an agent that never sees this workspace. **It owns the full project-side doctrine** — doc kinds + rates, the navigation discipline, the domain gate + chore/comet rule, SPEC anatomy + the graduation rule, walkthroughs + the intake/given/URD evidence doors + the plan sheet, the never-do list — **do not restate any of it here.** The split cuts both ways: workspace rules (tiers, junctions, `PROJECTS.md`) are never embedded in the card; protocol rules live only in the card. `check.py consistency` WARNs when a card is missing or stale. **The card has a human twin**: `.gravity/GRAVITY.html` (from `gravity/GRAVITY-GUIDE.html`, + the `.ko` Korean sibling) is the same protocol written for the engineer who just opened the repo — browser-read, SVG figures, a phrasebook of what to ask for. Same verbatim-copy contract, copied and re-copied in the same breath, and **only the card is stamped** (the family moves together, so one version covers all of it). This is the SPEC/ARCHITECTURE audience split applied to the protocol itself — never let a project carry some of the family without the rest.
...
| ⊙ `/given <name>` | Route `.gravity/_inbox/` into the given layer: one routing table (domain · fidelity · privacy), provenance manifests, inbox ends empty. |
