...
| One domain's enforceable rules (agent contract) | **`.gravity/<domain>/SPEC.md`** — the compact agent-loadable sheet, lint-enforced, links up to its facet | "is this the short rule set you'd hand an *agent* to make a change safely?" |
...
Same ownership rule as everywhere (one concern, one home): the **facet `ARCHITECTURE.html` owns the full human rationale**; the paired **`SPEC.md` owns the enforceable rules an agent loads** and links *up* to its ARCHITECTURE rather than restating it; add a `SPEC.md` **only for domains an agent actually changes** (read-only domains stay HTML-only). Enforce specs with a linter where the rules are checkable (`knowledge-viewer`'s `npm run lint:docs` is the reference implementation).
