...
├── SPEC.template.md                # Per-domain agent-loadable rule-sheet stencil, paired with an ARCHITECTURE facet (optional; §6).
...
| One facet's deep "how it's built" (human reference) | **`ARCHITECTURE.<facet>.html`** — when one ARCHITECTURE page serves several domains | "is this the full rationale a *human* engineer reads for one domain?" |
| One domain's enforceable rules (agent contract) | **`SPEC.<domain>.md`** — the compact agent-loadable sheet, lint-enforced, links up to its facet | "is this the short rule set you'd hand an *agent* to make a change safely?" |
...
**Optional: faceted deep-dives + agent specs — `ARCHITECTURE.<facet>.html` & `SPEC.<domain>.md` (the audience split on "how it's built").** When a single `ARCHITECTURE.html` outgrows one page — several domains each worth a real deep-dive — split it by **audience**, not just by topic. Two paired families:

- **`ARCHITECTURE.<facet>.html`** — a browser-read deep-dive for a *human engineer* on one facet (e.g. `ARCHITECTURE.doc.html`, `ARCHITECTURE.search.html`, `ARCHITECTURE.security.html`). The root `ARCHITECTURE.html` stays the system overview; each facet file carries the full rationale and key concepts for its domain. Deliberately *more* than an agent needs loaded into context.
- **`SPEC.<domain>.md`** — the compact, **agent-loadable** distillation of that facet's *rules and contract* (e.g. `SPEC.doc.md`, `SPEC.topic.md`). It is the file you hand an agent directly for a specific change. Keep it short: it names its canonical `ARCHITECTURE.<facet>.html` at the top and **must not grow into a second architecture document**. Enforce it with a linter where the rules are checkable (`knowledge-viewer`'s `npm run lint:docs` is the reference implementation).
- **`CLAUDE.md` is the router.** A *Document Authoring* section tells the agent *which `SPEC.*` to read before which kind of change* ("touching the parser or topic UI → read `SPEC.doc.md` first"), and which `ARCHITECTURE.*.html` is the human reference behind it.

Same ownership rule (one concern, one home): the **facet `ARCHITECTURE` owns the full rationale**; the paired **`SPEC` owns the enforceable rules an agent loads**; the SPEC **links up** to its ARCHITECTURE rather than restating it. Both are **recognized only when present and never mandated** (like `ARCHITECTURE.html`/`MISSION.html` — `/mission` reads them, `/triage` checks them for drift) — adopt the split only when one architecture page genuinely serves two audiences badly. **`knowledge-viewer` is the worked example.** Copy `SPEC.template.md` for the agent sheet; a facet HTML reuses `ARCHITECTURE.template.html`'s theme but drops its fixed section skeleton — a facet is a free-form deep-dive, not another system overview.
