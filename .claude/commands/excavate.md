...
2. **Check what docs exist**: root `CLAUDE.md` / `CONTEXT.md` / any `CONTRACT.md` / `GLOBAL_RULES.md` / `.gravity/`. Existing content is evidence — read it, never overwrite it.
3. **Inventory the runnable parts**: build manifests (`pom.xml`, `build.gradle`, `package.json`, `Dockerfile`, `docker-compose.*`) → one row per runnable service (name, stack, likely port). This table is the skeleton everything else hangs on.
...
Scan **code, not the database**: data-dictionary extraction (`ALL_TAB_COLUMNS` etc.) needs credentials and a read-only user — record it as an `OPEN:` follow-up, don't attempt it.
