...
Scan **code, never connect to the database**: live data-dictionary extraction needs credentials — the offline **DB evidence pack** is the sanctioned substitute. Consume only what its MANIFEST marks `present`; everything else stays an `OPEN:` follow-up. **When the user has no pack, guide — don't demand:** ask what they can actually obtain, cheapest first — (1) DDL they can scrap themselves (SQL Developer / DBeaver export, migration files) → `ddl/*.sql`, (2) the P1 dictionary CSVs (the DBA ask; queries in `DB-EVIDENCE.template.md`), (3) the rest opportunistically. Say explicitly that **no row data is ever needed** — the pack is the schema's own structure.
...
It accepts **either** graph source — `constraints.csv` from the live dictionary **or** `ddl/*.sql` the user scraped themselves (when both exist the dictionary wins and DDL disagreement is reported as drift). It builds the FK graph, partitions tables into candidate **vertical (business) domains** (naming prefix vs grant signature vs FK community — the winner chosen by modularity, printed so it can be argued with), and names the **candidate seams**: FK edges that leave their cluster, cross-schema first. Its honesty rules are the command's: absent files report `unknown` — never zero — and every confidence score names the signals it used. Treat its output as **evidence rows** (cite as `scan_db: <finding>` + the CSV behind it), its candidates as **proposals** for Step 5, and its seams as Boundary-Map input for Step 2's table-name join. When the code carries no readable queries (dynamic SQL, stored procedures, repos you can't see), this is where the DB half of the seams comes from.
...
- **Propose** candidate `.gravity/<domain>/` folders along both axes, **capabilities first**: vertical (business) candidates from what the code evidences — route groupings, module/package names, and (with a DB evidence pack) `scan_db.py`'s candidates, carried over with their confidence + `signals_used` so a 2-signal score is never presented as a 3-signal one — each citing its files/CSVs; then structural candidates (`web`/`api`/`data`/`security`) **only where a runtime owns real conventions worth fencing**. Never one folder per service/repo by default — the deployable topology is already the integration domain's Boundary Map, and re-listing it as domains plateaus the analysis at what the directory listing already said. All candidates are gated later by `/interview <project> <feature>` like any candidate. **Don't mint them** — the *is-it-a-domain* gate and `/new-domain` stay the user's move. Note: legacy modules enter the status spine as **✓ stable** (it's shipped, working software); a domain flips ◑ only when work lands on it.
