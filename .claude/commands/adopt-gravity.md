...
5. **Seed the router + fence the harness files + embed the protocol card.** Gravity's entire footprint in root files is a 4-line fenced block — everything else in them belongs to the project:
   ...
   - **The lib** — `python .claude/scripts/install_lib.py <project>` copies `gravity/lib/` into `.gravity/lib/` with a `VERSION` stamp. The card makes the repo self-*describing*; the lib makes it self-*rendering* — a clone with no workspace can scan, check and render its own observatory (`python .gravity/lib/generate_observatory.py`, no arguments). Same contract as the card: a verbatim copy, never hand-edited per project. It is **committed** (that's the point — it has to survive the clone); only its `__pycache__/` is ignored.
   - **The inbox door** — create `.gravity/inbox/`, copy `gravity/templates/INBOX.template.md` to `.gravity/inbox/README.md` (drop the template's top comment), and add to the project's `.gitignore`:
     ```
     .gravity/inbox/*
