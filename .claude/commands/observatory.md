...
allowed-tools: Read, Glob, Grep, Bash(python gravity/lib/generate_observatory.py:*), Bash(python .kepler/scripts/install_lib.py:*)
---
...
| **Timeline** | `docs/walkthroughs/` as a reverse-chron proof strip — date · domain chips (md `Domain(s):` header / html `gravity-domains` meta, filename fallback) · title, each linking to its file; both generations (.md sheets, .html debriefs) on one strip. Honest empty state pointing at `WALKTHROUGH.template.html` when the log doesn't exist. | native |
