...
1. **Resolve the project** via `.kepler/scripts/resolve_project.py`. Not found → list candidates and stop.
...
4. **Apply the mechanical layer** (skip this step entirely if the v4 migrator ran in step 3 — it already did all of it):
   ...
   - **Re-install the lib** — `python .kepler/scripts/install_lib.py <project>` overwrites `.gravity/_lib/` with the current modules and re-stamps its `VERSION`. Same contract as the card: a **verbatim copy, never hand-edited per project**, so upgrading means re-copying. The script prints `lib <old> -> <new>`; quote that line in the report.
