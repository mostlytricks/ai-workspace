...
2. **Read only what judgment needs** (skip what's missing): each project's `MISSION.html`, `IMPLEMENTATION_PLAN.md`, `ARCHITECTURE.html` — at the root, or under `.gravity/` via the **Doc Map** (`.gravity/ROUTER.md`; pre-v3: root `CLAUDE.md` — §6). Note the current phase and the Non-Goals list; skim MISSION against `CLAUDE.md` for the **doc-collision** check (step 7). Full `CONTEXT.md` reads are only needed where the report wants more than the scan's `next_step` line.
