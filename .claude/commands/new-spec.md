...
You are running `/new-spec` from `ai-workspace/` to write **`.gravity/<domain>/SPEC.md`** for project **`<project>`** (workspace CLAUDE.md §6; stencil = `gravity/templates/SPEC.template.md`). Parse `$ARGUMENTS` as `<project> <domain>`; if either is missing, ask.
